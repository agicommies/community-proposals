...
export default function ExpandedProposalView({ params }: { params: { slug: string } }) {
  ...
  const handleProposalsContent = () => {
    const proposal = proposals?.find((proposal) => proposal.id === paramsInfo.id)
  ...
  if (isLoading) return <div className="flex items-center justify-center w-full h-svh">
  ...
  return (
    <div className="flex flex-col max-w-6xl px-4 mx-auto ">
      <Link href={'/'} className="flex items-center justify-center gap-2 px-5 py-3 my-6 text-gray-400 border border-gray-500 w-fit hover:border-green-500 hover:text-green-500 ">
        <ArrowLeftIcon className="h-6 text-green-500" />
        Go Back to Proposals List
      </Link>
      <div className="flex flex-col justify-between w-full mb-6 text-white border border-gray-500 divide-gray-500 lg:divide-x lg:flex-row border-x-none xl:border-x ">
        <div className="flex flex-col lg:w-2/3 lg:h-[calc(100svh-203px)] lg:overflow-auto">
          ...
          <div className="h-full p-6 lg:overflow-auto">
            <MarkdownPreview source={content?.body ?? ''} style={{ backgroundColor: 'transparent', color: 'white' }} className={`${cairo.className}`} />
          </div>
        </div>
          ...
          {content && paramsInfo.contentType == 'proposal' && (
            <>
              ...
              <div className="w-full p-6 border-gray-500 lg:border-b ">
                {!content.stakeInfo && <span className="flex text-gray-400">
                  Loading results...
